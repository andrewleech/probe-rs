...
/// Timing breakdown for CRC32 verification
#[derive(Debug)]
struct Crc32Timing {
    host_time: Duration,
    algo_load_time: Duration,
...
impl Flasher {
    ...
    pub(super) fn program_incremental(
        ...
        if restore_unwritten_bytes {
            ...
            self.fill_unwritten(session, progress)?;
            tracing::info!("TIMING: Fill unwritten took {:.2}s", fill_start.elapsed().as_secs_f32());
        }
        ...
        tracing::info!("TIMING: Preparation took {:.2}s", prep_start.elapsed().as_secs_f32());
        ...
        tracing::info!("TIMING: Starting sector verification phase with on-chip CMSIS pc_verify");
        
        // Start CRC32 verification progress
        progress.started_crc32_verifying();
        
        // Track detailed verification timing
        let mut total_host_time = Duration::ZERO;
        let mut total_target_time = Duration::ZERO;
        let mut total_algo_load_time = Duration::ZERO;
        let mut sector_count = 0;
        
        for (sector_addr, region_page_indices) in &sector_pages {
            let sector_start = Instant::now();
            
            // Use CRC32 verification for 99% speed improvement
            let (sector_verification_passed, timing) = match self.verify_sector_crc32(
                session,
                *sector_addr,
                region_page_indices,
            ) {
            ...
            // Accumulate timing data for performance analysis
            total_host_time += timing.host_time;
            total_algo_load_time += timing.algo_load_time;
            total_target_time += timing.target_time;
            sector_count += 1;
            ...
            if sector_needs_update {
                ...
                updated_sectors += 1;
                tracing::info!("🔄 TIMING: Sector 0x{:08x} needs update - CRC32 mismatch (verified in {:.1}ms)", 
                    sector_addr, sector_time.as_secs_f32() * 1000.0);
            } else {
                verified_sectors += 1;
                tracing::info!("✅ TIMING: Sector 0x{:08x} verified OK - CRC32 match (verified in {:.1}ms)", 
                    sector_addr, sector_time.as_secs_f32() * 1000.0);
            }
        }
        
        let total_verify_time = verify_start.elapsed();
        tracing::info!("TIMING: Total verification phase took {:.2}s", total_verify_time.as_secs_f32());
        tracing::info!("📈 VERIFICATION SUMMARY: {} of {} sectors need updates ({:.1}%)", 
        ...
        if sector_count > 0 {
            ...
            tracing::info!("  📊 Total sectors: {}", sector_count);
            tracing::info!("  📊 Host CRC32 time: {:.1}ms total ({:.2}ms avg/sector)", 
                total_host_time.as_millis(), total_host_time.as_millis() as f64 / sector_count as f64);
            tracing::info!("  📊 Algorithm load time: {:.1}ms total ({:.2}ms avg/sector)", 
                total_algo_load_time.as_millis(), total_algo_load_time.as_millis() as f64 / sector_count as f64);
            tracing::info!("  📊 Target CRC32 time: {:.1}ms total ({:.2}ms avg/sector)", 
                total_target_time.as_millis(), total_target_time.as_millis() as f64 / sector_count as f64);
            
            let overhead_time = total_verify_time - total_host_time - total_target_time - total_algo_load_time;
            tracing::info!("  📊 Communication overhead: {:.1}ms total ({:.2}ms avg/sector)", 
                overhead_time.as_millis(), overhead_time.as_millis() as f64 / sector_count as f64);
            tracing::info!("  📊 Performance: {:.1}ms/sector total ({:.1} sectors/second)", 
                total_verify_time.as_millis() as f64 / sector_count as f64, 
                sector_count as f64 / total_verify_time.as_secs_f64());
        }
        ...
        if !sectors_to_update.is_empty() {
            ...
            tracing::info!("TIMING: Starting erase phase for {} of {} sectors ({:.1}%)", 
                sectors_to_update.len(), total_sectors,
                (sectors_to_update.len() as f64 / total_sectors as f64) * 100.0);
            ...
            tracing::info!("TIMING: Selective erase took {:.2}s", erase_start.elapsed().as_secs_f32());
        }

        // Phase 3: Program sectors using persistent buffer optimization where available
        if !sectors_to_update.is_empty() {
            let program_start = Instant::now();
            tracing::info!("TIMING: Starting programming phase for {} sectors", sectors_to_update.len());
            ...
            tracing::info!("TIMING: Programming took {:.2}s", program_start.elapsed().as_secs_f32());
        } else {
            tracing::info!("TIMING: No sectors needed updating - all sectors verified successfully");
        }

        // Phase 4: Final verification if requested
        let final_verify_start = Instant::now();
        if verify {
            tracing::info!("TIMING: Starting final verification phase");
            if !self.verify(session, progress, !restore_unwritten_bytes)? {
                return Err(FlashError::Verify);
            }
            tracing::info!("TIMING: Final verification took {:.2}s", final_verify_start.elapsed().as_secs_f32());
        }

        let total_time = start_time.elapsed();
        tracing::info!(
            "TIMING: Incremental flash complete in {:.2}s: {} sectors verified, {} sectors updated", 
        ...
        tracing::info!("🎯 FINAL PERFORMANCE SUMMARY:");
        tracing::info!("  🎯 Total execution time: {:.2}s", total_time.as_secs_f64());
        if total_sectors > 0 {
            tracing::info!("  🎯 Throughput: {:.1} sectors/second", total_sectors as f64 / total_time.as_secs_f64());
            tracing::info!("  🎯 Data verified: {:.1}KB ({:.1}KB/s)", 
                total_sectors as f64 * 4.0, 
                (total_sectors as f64 * 4.0) / total_time.as_secs_f64());
        }
    ...
    fn verify_sector_crc32(
        ...
        tracing::debug!("🧮 CRC32: Starting verification for sector 0x{:08x}", sector_addr);
        
        // Step 1: Calculate host-side CRC32 of expected data
        let host_crc_start = Instant::now();
        ...
        let host_crc_time = host_crc_start.elapsed();
        
        // Step 2: Load CRC32 algorithm to target RAM if not already loaded
        let algo_load_start = Instant::now();
        let crc32_algo_addr = self.ensure_crc32_algorithm_loaded(session)?;
        let algo_load_time = algo_load_start.elapsed();
        
        // Step 3: Execute CRC32 calculation on target flash memory
        let target_crc_start = Instant::now();
        ...
        let target_crc_time = target_crc_start.elapsed();
        
        let total_time = start_time.elapsed();
        let verification_passed = host_crc32 == target_crc32;
        
        tracing::info!(
            "🧮 CRC32: Sector 0x{:08x} - host=0x{:08x} target=0x{:08x} [{}] ({:.1}ms total: {:.1}ms host, {:.1}ms load, {:.1}ms target)",
            sector_addr, host_crc32, target_crc32,
            if verification_passed { "PASS" } else { "FAIL" },
            total_time.as_secs_f32() * 1000.0,
            host_crc_time.as_secs_f32() * 1000.0,
            algo_load_time.as_secs_f32() * 1000.0,
            target_crc_time.as_secs_f32() * 1000.0
        );
        
        let timing = Crc32Timing {
            host_time: host_crc_time,
            algo_load_time,
            target_time: target_crc_time,
        };
    ...
    fn ensure_crc32_algorithm_loaded(&mut self, session: &mut Session) -> Result<u64, FlashError> {
        ...
        // Load the appropriate CRC32 binary blob for target architecture
        let (crc32_blob, algo_type) = self.get_crc32_algorithm_blob(session)?;
        tracing::debug!("🧮 Loading {} CRC32 algorithm ({} bytes) to 0x{:08x}", algo_type, crc32_blob.len(), CRC32_ALGO_ADDR);
    ...
    fn get_crc32_algorithm_blob(&self, session: &Session) -> Result<(Vec<u8>, &'static str), FlashError> {
        let target = session.target();
        let arch = &target.architecture();
        
        match arch {
            probe_rs_target::Architecture::Arm => {
                // Check if we should use high-performance slicing-by-4 variant
                if self.should_use_slicing_by_4(target) {
                    const ARM_THUMB_SLICE4_BLOB: &[u8] = include_bytes!("crc32-blobs/arm-thumb/crc32_slice4_optimized.bin");
                    tracing::info!("🧮 Using ARM Thumb slicing-by-4 CRC32 blob ({} bytes) for high-performance targets", ARM_THUMB_SLICE4_BLOB.len());
                    Ok((ARM_THUMB_SLICE4_BLOB.to_vec(), "arm-thumb-slice4"))
                } else {
                    const ARM_THUMB_CRC32_BLOB: &[u8] = include_bytes!("crc32-blobs/arm-thumb/crc32.bin");
                    tracing::debug!("🧮 Using ARM Thumb standard CRC32 blob ({} bytes)", ARM_THUMB_CRC32_BLOB.len());
                    Ok((ARM_THUMB_CRC32_BLOB.to_vec(), "arm-thumb"))
                }
            },
            probe_rs_target::Architecture::Riscv => {
                const RISCV_CRC32_BLOB: &[u8] = include_bytes!("crc32-blobs/riscv/crc32.bin");
                tracing::info!("🧮 Using RISC-V slicing-by-4 CRC32 blob ({} bytes) for ESP32-C3/C6 and CH32V targets", RISCV_CRC32_BLOB.len());
                Ok((RISCV_CRC32_BLOB.to_vec(), "riscv"))
            },
            probe_rs_target::Architecture::Xtensa => {
                // Xtensa support planned but not yet implemented
                tracing::warn!("🧮 Xtensa CRC32 support not yet implemented, falling back to ARM Thumb algorithm");
                const ARM_THUMB_CRC32_BLOB: &[u8] = include_bytes!("crc32-blobs/arm-thumb/crc32.bin");
                Ok((ARM_THUMB_CRC32_BLOB.to_vec(), "arm-thumb-fallback"))
            },
        }
    }
    
    /// Determine if we should use the high-performance slicing-by-4 algorithm
    /// Based on available RAM and target capabilities
    fn should_use_slicing_by_4(&self, target: &crate::config::Target) -> bool {
        // Slicing-by-4 requires 4KB lookup tables vs 1KB for standard CRC32
        // Only use on targets with sufficient RAM
        const MIN_RAM_FOR_SLICE4: u64 = 16 * 1024; // 16KB minimum
        
        let total_ram = target.memory_map
            .iter()
        ...
        let has_sufficient_ram = total_ram >= MIN_RAM_FOR_SLICE4;
        
        if has_sufficient_ram {
            tracing::info!("🧮 Target has {}KB RAM, enabling slicing-by-4 CRC32 for 6-8x performance", total_ram / 1024);
            true
        } else {
            tracing::info!("🧮 Target has {}KB RAM, using standard CRC32 algorithm", total_ram / 1024);
            false
        }
    }
